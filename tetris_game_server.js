...
function configure_protocol_on_each_connection(socket){
    ...
    socket.on('ready', (socket_id_to_play_with) => { // user is 'ready to play'
        let c1 = connections_by_socket(socket);
        let c2 = connections_by_socket_id(socket_id_to_play_with);
        ...
        let a=[], types_of_bricks = 7, list_length = 250;
    ...
    socket.on('msg', (data) => {
        var c1 = connections_by_socket(socket);
        var c2 = connections_by_socket(c1.partner_socket);
        if (!c2)
        {
	...
	socket.on('score', (payload) => {
        let c1 = connections_by_socket(socket);
        let c2 = connections_by_socket(c1.partner_socket);
        if(!c1 || !c2)
            return;
    ...
    socket.on('chat', (msg) => {
        ...
        else
        {
            ...
            let waiting_connections = connections.filter(c => 
...
function send_scores(socket){
    ...
    if(!c1 || !c2)
        return;
    let user1 = users_get(c1.username), user2 = users_get(c2.username);
    let s1 = user1.score, r1 = user1.rank;
    let s2 = user2.score, r2 = user2.rank;
...
function users_by_socket_id(socket_id){
    return connections_by_socket_id(socket_id).username;
}
